...
```
AI Tools (Codex, ChatGPT, Cursor) ──via MCP──> FlashQuery ──> Supabase (memory, vectors, relational)
                                                                ──> Local vault (markdown files, Obsidian-compatible)
```

FlashQuery runs as a server process started from the CLI. It uses **stdio MCP transport** (Codex Desktop/Code spawns it as a subprocess). It connects to a local or hosted Supabase instance for relational data and vector search, and manages a local folder of markdown files that doubles as an Obsidian vault.
