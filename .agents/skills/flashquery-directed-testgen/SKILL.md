...
Batch mode assumes you can spawn parallel subagents (which works in Codex and Cowork). If you're running in an environment that doesn't support subagents, batch mode degrades gracefully to sequential execution: run the per-test workflow (phases 2–6) for each test plan one at a time in the parent context, then do phases B3–B5 normally at the end.
