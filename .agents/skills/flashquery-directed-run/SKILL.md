...
description: Run FlashQuery directed scenario tests, from one test to the full suite, then triage failures and route them to the right follow-up. Use this skill when the user wants to execute directed tests, investigate directed test failures, triage suite results, debug the directed suite, run run_suite.py, or asks to "run the suite", "run these tests", "why are these failing", "triage failures", "work through failing tests", or "check if everything still passes". This is the runner counterpart to flashquery-directed-testgen and flashquery-directed-covgen.
---
...
- `.Codex/skills/flashquery-directed-testgen/SKILL.md` — where you hand off for confirmed test bugs
- `.Codex/skills/flashquery-directed-covgen/SKILL.md` — where you hand off for confirmed coverage-definition issues
...
**Detecting availability:** check for an agent definition at `.Codex/agents/gsd-debug.md` or a slash command at `.Codex/commands/gsd-debug.md` (or similar project-local conventions). If present, treat `/gsd-debug` as the preferred route for FlashQuery defects.
