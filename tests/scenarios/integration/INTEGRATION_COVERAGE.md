...
| IL-03  | Multiple call_model calls sharing a trace_id accumulate distinct rows in fqc_llm_usage; trace_cumulative.total_calls grows monotonically (COST-01)            | llm_cost_accumulation        | 2026-05-05   | 2026-05-05   |
...
| IL-10  | call_model with {{ref:path}} placeholder injects document body before LLM dispatch; response metadata includes injected_references[] and prompt_chars (REF-01, REF-07) | test_call_model_reference_system_core; reference-resolver.integration.test.ts | 2026-05-05   | 2026-05-05   |
| IL-11  | call_model with {{ref:path#Section}} injects only the named section's content (REF-02)                                                                              | test_call_model_reference_system_core; reference-resolver.integration.test.ts | 2026-05-05   | 2026-05-05   |
| IL-12  | call_model with {{ref:path->pointer}} dereferences frontmatter pointer; response metadata includes resolved_to (REF-02, REF-07)                                     | test_call_model_reference_system_core; reference-resolver.integration.test.ts | 2026-05-05   | 2026-05-05   |
| IL-13  | call_model with unresolvable or invalid {{ref:...}} returns isError + reference_resolution_failed with stable reason/detail; no LLM call made (REF-04, REF-06)       | test_call_model_reference_system_core; reference-resolver.integration.test.ts | 2026-05-05   | 2026-05-05   |
| IL-14  | call_model without active {{ref:...}} patterns omits injected_references and prompt_chars; active and escaped {{id:...}} remain literal ATL v1 text (REF-03, REF-05) | test_call_model_reference_system_core | 2026-05-05   | 2026-05-05   |
| IL-15  | call_model resolver=list_models returns {models: [...]} where each entry has name, type, provider, model_id, input_cost_per_million, output_cost_per_million (DISC-01 required fields), tags, and structured capabilities when declared              | llm_discovery_list           | 2026-05-06   | 2026-05-06   |
| IL-16  | list_models discovery → model names match config → call_model with one of those names succeeds | llm_discovery_then_call      | 2026-05-07   | 2026-05-07   |
| IL-17  | list_purposes discovery → purpose names match config → call_model by purpose succeeds | llm_discovery_then_call      | 2026-05-07   | 2026-05-07   |
| IL-18  | call_model resolver=list_models and resolver=list_purposes both succeed when called WITHOUT name and WITHOUT messages — schema makes them optional for discovery (DISC-04)                            | llm_discovery_list           | 2026-05-06   | 2026-05-06   |
...
| IL-21  | call_model with {{ref:...}} writes a fqc_llm_usage row reflecting the resolved (post-injection) prompt → get_llm_usage summary total_calls increments by 1; recorded prompt size aligns with metadata.prompt_chars | llm_ref_writes_usage_row     | 2026-05-04   | 2026-05-04   |
...
| IL-37  | ATL-INT-01: Reference/template freshness after document update; a subsequent `call_model` reference/template flow sees the updated marker and not the original marker | llm_template_reference_freshness | 2026-05-07   | 2026-05-07   |
| IL-38  | ATL-INT-02: Document-parameter freshness after referenced document update; template rendering uses the updated document parameter target | llm_template_document_param_freshness | 2026-05-07   | 2026-05-07   |
| IL-39  | ATL-INT-03: Discovery-to-invocation closure from public `list_purposes` discovery to a subsequent `call_model` purpose invocation | llm_discovery_then_call      | 2026-05-07   | 2026-05-07   |
| IL-40  | ATL-INT-05: Mixed path, section, pointer, alias, and `_items` template/reference modes compose in one `call_model` flow without active `{{id:...}}` placeholders | llm_mixed_reference_modes    | 2026-05-07   | 2026-05-07   |
