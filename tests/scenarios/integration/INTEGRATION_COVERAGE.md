...
| IL-16  | list_models discovery → model names match config → call_model with one of those names succeeds | llm_discovery_list           | 2026-05-06   | 2026-05-06   |
| IL-17  | list_purposes discovery → purpose names match config → call_model by purpose succeeds | llm_discovery_list           | 2026-05-06   | 2026-05-06   |
...
| IL-37  | ATL-INT-01: Reference/template freshness after document update; a subsequent `call_model` reference/template flow sees the updated marker and not the original marker | llm_template_reference_freshness | 2026-05-07   |              |
| IL-38  | ATL-INT-02: Document-parameter freshness after referenced document update; template rendering uses the updated document parameter target | llm_template_document_param_freshness | 2026-05-07   |              |
| IL-39  | ATL-INT-03: Discovery-to-invocation closure from public `list_purposes` discovery to a subsequent `call_model` purpose invocation | llm_discovery_then_call | 2026-05-07   |              |
| IL-40  | ATL-INT-05: Mixed path, section, pointer, alias, and `_items` template/reference modes compose in one `call_model` flow without active `{{id:...}}` placeholders | llm_mixed_reference_modes | 2026-05-07   |              |
