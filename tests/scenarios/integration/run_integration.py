...
def _substitute(value: Any, variables: dict[str, dict]) -> Any:
    ...
    if isinstance(value, str):
        ...
        if full_match:
            name, field = full_match.group(1), full_match.group(2)
...
def run_yaml_test(
    ...
    variables: dict[str, dict] = {
        ...
        # Built-in: ${test.llm_model} — model alias generated by the managed
        # LLM fixture from .env.test / environment values.
        "test": {
            "llm_model": test_llm_model_name(
                Path(args.fqc_dir) if getattr(args, "fqc_dir", None) else None
            ),
            "llm_timeout_ms": test_llm_timeout_ms(
                Path(args.fqc_dir) if getattr(args, "fqc_dir", None) else None
            ),
        },
    }
