...
- `.env` or `.env.test` — environment variables referenced as `${VAR}` in the YAML config. At a minimum, this needs to contain the Supabase credentials (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `DATABASE_URL`) and the auth secret (`FQC_AUTH_SECRET`). If any managed tests use `require_embedding=True`, the harness generates an `llm.purposes[name=embedding]` config from `FQC_TEST_EMBEDDING_MODE`, `FQC_TEST_EMBEDDING_DIMENSIONS`, `OLLAMA_URL`, `OLLAMA_EMBEDDING_MODEL`, `OPENAI_EMBEDDING_API_KEY` / `OPENAI_API_KEY`, and `OPENAI_EMBEDDING_MODEL`. If any managed tests use `require_llm=True`, the harness generates an LLM config from `FQC_TEST_LLM_MODE`, `FQC_TEST_LLM_MODEL_ALIAS`, `FQC_TEST_LLM_PURPOSE`, `OLLAMA_URL`, `OLLAMA_LLM_MODEL`, `OPENAI_API_KEY`, `OPENAI_LLM_MODEL`, and `OPENAI_LLM_ENDPOINT`. If you're already running FlashQuery locally for development, you almost certainly have these files already.
...
| `require_llm=True` | Test exercises real `call_model` execution through the managed server's generated purpose. Defaults to local Ollama via `FQC_TEST_LLM_MODE=ollama`, with `FQC_TEST_LLM_MODEL_ALIAS=fast` and `FQC_TEST_LLM_PURPOSE=general`. |
