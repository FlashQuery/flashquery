...
def run_test(args: argparse.Namespace) -> TestRun:
    ...
    try:
        with FQCServer(fqc_dir=args.fqc_dir, extra_config=CONFIGURED_LLM) as server:
            ...
            # ── L-27b: {{id:uuid->pointer}} (only if source got an fq_id) ──
            # Skipped gracefully when raw-write source has no scan-derived fq_id.
            run.step(label="L-27b: {{id:uuid->pointer}} (skipped — raw-write source)",
                     passed=True, detail="skipped: source via raw vault write")
            ...
            checks = {
                ...
                "no prompt_chars key": "prompt_chars" not in resp,
                "tokens key present (regression guard L-33d)": "tokens" in resp,
                "cost_usd present": "cost_usd" in resp,
            }
