...
Scenario:
    ...
    4. (RO-63) Write 1100 .md files OUTSIDE the watched folder with fq_type
...
def _write_plain_md_files(dest_dir: Path, count: int, prefix: str) -> int:
    """Write `count` minimal .md files with pre-assigned fq_ids into dest_dir.

    Pre-assigning fq_ids means the scanner does a simple upsert without
    needing to write fq_id back to the file. This avoids filesystem
...
def _write_typed_md_files(dest_dir: Path, count: int, prefix: str, fq_type: str) -> int:
    """Write `count` .md files with fq_type frontmatter and pre-assigned fq_ids."""
    dest_dir.mkdir(parents=True, exist_ok=True)
    written = 0
    for i in range(count):
        p = dest_dir / f"{prefix}_{i:05d}.md"
        fq_id = str(uuid4())
        p.write_text(
            f"---\n{FM.ID}: {fq_id}\n{FM.TITLE}: Typed Scale Test {prefix} {i}\n{FM.STATUS}: active\n"
            f"{FM.TYPE}: {fq_type}\n---\n\n"
...
def run_test(args: argparse.Namespace) -> TestRun:
    ...
    with TestContext(
        ...
        # The outside docs have fq_type in frontmatter; the scanner should have
        ...
        if not ro63_ok and total_added_63 == 1000:
            ...
            ro63_verdict_detail += (
                ...
                f"verify {FM.TYPE} frontmatter was written and scan indexed the files"
            )
