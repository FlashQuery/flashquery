...
**`require_llm=True`** — needed for tests that make real `call_model` requests and should use the managed server's generated purpose. `FQC_TEST_LLM_MODE` supports `ollama` (default) and `openai`; `FQC_TEST_LLM_MODEL_ALIAS` and `FQC_TEST_LLM_PURPOSE` set the generated model/purpose names; `OLLAMA_URL` / `OLLAMA_LLM_MODEL` configure the local provider; `OPENAI_API_KEY`, `OPENAI_LLM_MODEL`, and `OPENAI_LLM_ENDPOINT` configure the OpenAI-compatible provider. Prefer this over hardcoding OpenAI in tests so the suite can run fully off-grid with Ollama.
