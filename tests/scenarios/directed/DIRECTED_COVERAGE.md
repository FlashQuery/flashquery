...
- L-87: ATL-DS-12 directed budget behavior covers max-token, max-cost, max-iteration, wall-clock timeout, provider-error stop, zero public usage rows when no iteration completes, and completed-iteration accounting. Three-checkpoint timeout differentiation (pre-LLM-call / pre-tool-dispatch / post-tool-dispatch), estimate ladders, and per-model fallback cost are validated by the Phase 117 unit/E2E gates rather than this directed testcase. Cooperative shutdown is tracked as L-90 (pending — see future-test row).
