...
| L-66 | `call_model` with `resolver=list_models` for a model configured with `local: true` in the YAML preserves that flag verbatim in the response entry (`response.models[i].local === true`). Configure a model with `local: true` declared at the top level of its model entry; assert the `local` key is present with value `true`. **⚠ NOT YET LOCATED IN CODE** — spec §8.3 Ollama example shows `"local": true` but [src/mcp/tools/llm.ts:163-180](../../../src/mcp/tools/llm.ts#L163-L180) field mapping does not currently pass `local` through. This row is asserted as the intended contract per the spec; expect this test to FAIL on initial run if the field mapping has not been added — that failure is the actionable signal to either ship the feature or update the spec. | — | 2026-05-03 |  |
...
| L-68 | `get_llm_usage` `mode=summary` aggregation arithmetic — `total_spend_usd` exactly equals the sum of `cost_usd` across every matched row within absolute tolerance ±1e-9. Setup: seed exactly N=3 `call_model` calls within the queried period, capturing each response's `metadata.cost_usd` (`c1`, `c2`, `c3`); call `get_llm_usage` with `mode=summary` and a period covering all three; assert `response.total_spend_usd` is within ±1e-9 of `c1 + c2 + c3`. Tightens L-18 (which only verifies `total_spend_usd` is a number ≥ 0) to actual aggregation correctness — catches a regression that double-counts or drops rows. | — | 2026-05-03 |  |
