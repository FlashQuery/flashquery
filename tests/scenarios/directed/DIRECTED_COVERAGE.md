...
| L-46 | `get_llm_usage` round-trip cost equality — after a successful `call_model` returns `metadata.cost_usd: X` and `metadata.trace_id: T`, calling `get_llm_usage` with `mode=recent`, `limit=1`, `trace_id=T` returns one entry whose `cost_usd` field is byte-for-byte equal to `X` (`response.entries[0].cost_usd === X`, strict `===`, no float coercion drift). Verifies the persisted Supabase row preserves cost precision through the storage round-trip. | test_get_llm_usage_round_trip* | 2026-05-03 | FAIL (2026-05-03) |
...
| L-51 | `call_model` with `{{ref:source.md->ptr}}` where `source.md` frontmatter contains `ptr: "target.md"` (bare filename — no `/`, ends with `.md`, but no UUID format) — call succeeds, `metadata.injected_references[0].resolved_to` is the vault-relative path of the matched target document (not the bare filename). Verifies the filename-search branch of §6.6 fires for `->` derefs in `call_model`. Parallels D-62 on the call_model surface. | test_call_model_pointer_resolutions* | 2026-05-03 | FAIL (2026-05-03) |
...
| L-55 | `call_model` with `{{ref:source.md->ptr}}` where `source.md` frontmatter has `ptr: 42` (or `ptr: ["a", "b"]`, or any non-string scalar/object) — returns `isError: true` with `error: "reference_resolution_failed"`. The response's `failed_references[0].reason` contains the substring `"not a string"` (per §6.5 "value at path is not a string" failure mode). Verifies the non-string-validation branch of §6.5 + OQ #11. | test_call_model_ref_error_taxonomy* | 2026-05-03 | FAIL (2026-05-03) |
...
### test_get_llm_usage_round_trip ⚠ FAIL (2026-05-03) — L-46

**Behaviors affected**
- L-46: `get_llm_usage` round-trip cost equality (strict `===` equality between `metadata.cost_usd` returned by `call_model` and `entries[0].cost_usd` returned by `get_llm_usage` for the same trace).

**Description**: The test seeds one `call_model(resolver=model, name=fast, trace_id=T)` with non-trivial decimal rates (0.59 / 0.79), captures the envelope's `metadata.cost_usd`, sleeps 3s for the fire-and-forget row to commit, then queries `get_llm_usage(mode=recent, limit=1, trace_id=T)` and compares the two `cost_usd` values with strict Python `==`. The two values are NOT byte-for-byte equal because the envelope value carries IEEE-754 binary FP noise (e.g. `0.000009640000000000001`) while the persisted Supabase row is stored in `NUMERIC(18,10)` (per `src/storage/supabase.ts:511`) which strips the FP tail to `0.00000964`. Mathematically the DB value is the *correct* one (15 input × 0.59/M + 1 output × 0.79/M = 9.64e-6 exactly), but it does not equal the in-flight envelope's float.

**Possible fixes** (deferred — pick one and investigate):
- (a) Round/normalize `call_model`'s `metadata.cost_usd` to NUMERIC(18,10) precision *before* returning the envelope, so envelope and DB return identical decimals end-to-end.
- (b) Soften the L-46 spec to "equal within ±1e-9 tolerance" (matches L-47's approach for `total_cost_usd`).

**Status**: Test is correctly written and stays in the suite as a regression detector for whichever resolution path is taken. L-67 in the same test file passes cleanly.

---

### test_call_model_pointer_resolutions ⚠ FAIL (2026-05-03) — L-51

**Behaviors affected**
- L-51: `call_model` with `{{ref:source.md->ptr}}` where `source.md` frontmatter contains `ptr: "target.md"` (bare filename, no `/`, ends with `.md`) — call should succeed with `injected_references[0].resolved_to` set to the matched target's vault-relative path.

**Description**: The test creates a target document `target51.md`, points `source51.md`'s frontmatter `ptr` at the bare filename `"target51.md"`, then calls `call_model` with `{{ref:source51.md->ptr}}`. FlashQuery returns `error: "reference_resolution_failed"` with reason `follow_ref target 'target51.md' (...) not found in vault: Document not found: "target51.md"`. The codebase intentionally treats any value ending in a configured markdown extension as path-only (per `test_follow_ref_get_document.py:209-211`, "post-Deviation 5, values ending in any configured markdown extension classify as 'path'") and does NOT fall back to filename matching when path lookup fails. This is a divergence between the L-51 spec (filename-search branch should fire) and the implemented contract.

**Possible fixes** (deferred):
- (a) Implement filename-fallback in `follow_ref` when the bare-filename pointer value fails as a path lookup, matching the L-51 spec and parallel to D-62 on the `get_document` surface.
- (b) Update the L-51 spec to acknowledge the path-only classification and remove the bare-filename branch from `follow_ref` semantics.

**Status**: Test is correctly written and stays in the suite as a regression detector. L-49, L-50, L-52, L-53 in the same test file pass.

---

### test_call_model_ref_error_taxonomy ⚠ FAIL (2026-05-03) — L-55

**Behaviors affected**
- L-55: `call_model` with `{{ref:source.md->ptr}}` where `source.md` frontmatter has `ptr: 42` (or any non-string scalar) — `failed_references[0].reason` should contain the substring `"not a string"`.

**Description**: FlashQuery's actual reason text is `"Reference path 'ptr' resolved to a number, expected a string identifier"`. This is semantically equivalent to "value at path is not a string" but does not contain the spec-required substring `"not a string"`. The test assertion as written by the spec fails on the substring match. All other reason-taxonomy assertions in the same test file (L-54, L-56, L-57a/b, L-58, L-61, L-62) pass with their actual reason strings.

**Possible fixes** (deferred — pick one):
- (a) Normalize FlashQuery's non-string pointer-value error wording to include `"not a string"` (would also unify the taxonomy across number/array/object value types — currently only the number case is covered).
- (b) Loosen the L-55 spec substring to `"expected a string"` or `"resolved to a"` to match the actual implementation while still pinning the meaning.

**Status**: Test is correctly written and stays in the suite as a regression detector. The other 6 IDs in the same test file pass.
