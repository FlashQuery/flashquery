...
describe.skipIf(!HAS_SUPABASE)('doctor embedding retry diagnostics', () => {
  ...
  it('T-I-006 reports embedding-null rows that lack pending retry state without raw embed text', async () => {
    ...
    await client.query(
      `
      INSERT INTO fqcp_phase146_doctor_records (id, instance_id, name, status, embedding)
    ...
    await client.query(
      ...
      INSERT INTO fqc_pending_embeds
        (instance_id, target_kind, target_table, target_id, target_label, embed_text, attempt_count, status)
      VALUES ($1, 'document', 'fqc_documents', $2, 'Tracked Gap', 'raw pending embed text', 1, 'pending')
      `,
      [TEST_INSTANCE_ID, docTrackedId]
    );
