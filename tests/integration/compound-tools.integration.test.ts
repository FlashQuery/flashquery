...
describe.skipIf(SKIP)('Compound Tools Integration', () => {
  ...
  describe('get_doc_outline', () => {
    let outlineDocPath: string;
    let batchDoc2Path: string;

    beforeAll(async () => {
      outlineDocPath = '_global/outline-test.md';
      const body = [
        '# Top Level Heading',
        '',
        'Some prose that must NOT appear in the outline output.',
        '',
        '## Section One',
        '',
        'More prose. See [[Related Note]] for details.',
        '',
        '### Subsection 1.1',
        '',
        ...
        '## Section Two',
        '',
        'Mentions [[Another Note|alias]] and ends here.',
      ].join('\n');

      await seedDocument({
        vaultPath,
        relPath: outlineDocPath,
        title: 'Outline Test Document',
        body,
        frontmatter: {
      ...
      batchDoc2Path = '_global/batch-test-2.md';
      await seedDocument({
        vaultPath,
    ...
    it('T2-06a: returns frontmatter fields and heading hierarchy without body prose', async () => {
      const result = await handlers('get_doc_outline')({ identifiers: outlineDocPath });

      expect(isError(result)).toBe(false);
      const text = getText(result);

      // Headings present
      expect(text).toContain('Top Level Heading');
      expect(text).toContain('Section One');
      expect(text).toContain('Subsection 1.1');
      expect(text).toContain('Section Two');

      // Frontmatter fields present
      expect(text).toContain('Outline Test Document');
      expect(text).toContain('Frontmatter:');

      // Body prose absent
      expect(text).not.toContain('Some prose that must NOT appear');
      expect(text).not.toContain('Even more prose here');
    });

    it('T2-06b: extracts wikilinks from body content and frontmatter values', async () => {
      const result = await handlers('get_doc_outline')({ identifiers: outlineDocPath });
      ...
      // Body wikilinks
      expect(text).toContain('Related Note');
    ...
    it('T2-06c: batch get_doc_outline returns DB metadata with === path === delimiters', async () => {
      const result = await handlers('get_doc_outline')({ identifiers: [outlineDocPath, batchDoc2Path] });
    ...
    it('T2-06d: batch get_doc_outline does not include headings (DB-only, no file I/O)', async () => {
      ...
      expect(text).not.toContain('Section One');
      expect(text).not.toContain('Subsection 1.1');
      expect(text).not.toContain('Another Heading');
      expect(text).not.toContain('Linked documents');
      expect(text).not.toContain('Frontmatter:');
    });

    it('T2-06e: single get_doc_outline on untracked file triggers auto-provisioning', async () => {
      const untrackedRelPath = '_global/untracked-outline.md';
      ...
      expect(isError(result)).toBe(false);
      const text = getText(result);
      expect(text).toContain('Untracked Outline Doc');
