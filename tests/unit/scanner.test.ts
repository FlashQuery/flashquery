...
// ─────────────────────────────────────────────────────────────────────────────
// Tests: Discovery Queue Building (Phase 54)
// ─────────────────────────────────────────────────────────────────────────────

describe('Discovery Queue Building', () => {
  let getFolderClaimsMapMock: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    vi.clearAllMocks();
    getFolderClaimsMapMock = vi.fn().mockReturnValue(new Map());
  });

  it('builds discoveryQueue for documents in plugin-claimed folders', async () => {
    const { getFolderClaimsMap } = await import('../../src/plugins/manager.js');
    getFolderClaimsMapMock = vi.mocked(getFolderClaimsMap);

    const folderMap = new Map([
      ['crm/contacts', { pluginId: 'crm', typeId: 'contact' }],
    ]);
    getFolderClaimsMapMock.mockReturnValue(folderMap);

    // Mock vault files
    vi.mocked(fsPromises.readdir as any).mockResolvedValue([
      { name: 'Sarah.md', isFile: () => true, isDirectory: () => false } as any,
    ]);
    ...
    // Mock DB documents
    const mockSupabase = supabaseManager.getClient();
    vi.mocked(mockSupabase.from as any).mockImplementation((table: string) => {
      if (table === 'fqc_documents') {
        return {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockReturnThis(),
    ...
    expect(result.discoveryQueue).toBeDefined();
    expect(result.discoveryQueue.length).toBeGreaterThan(0);
    expect(result.discoveryQueue[0].path).toBe('CRM/Contacts/Sarah.md');
    expect(result.discoveryQueue[0].pluginId).toBe('crm');
  });

  it('excludes documents with existing ownership_plugin_id from discoveryQueue', async () => {
    const { getFolderClaimsMap } = await import('../../src/plugins/manager.js');
    getFolderClaimsMapMock = vi.mocked(getFolderClaimsMap);
    ...
    vi.mocked(mockSupabase.from as any).mockImplementation((table: string) => {
      if (table === 'fqc_documents') {
        return {
          ...
          single: vi.fn().mockResolvedValue({
            data: { ownership_plugin_id: 'crm' }, // Already owned
            error: null,
          }),
        };
      }
    ...
    expect(result.discoveryQueue).toBeDefined();
    expect(result.discoveryQueue.length).toBe(0);
  });

  it('returns empty discoveryQueue when no plugins claim folders', async () => {
    const { getFolderClaimsMap } = await import('../../src/plugins/manager.js');
    getFolderClaimsMapMock = vi.mocked(getFolderClaimsMap);

    getFolderClaimsMapMock.mockReturnValue(new Map()); // No claimed folders

    vi.mocked(fsPromises.readdir as any).mockResolvedValue([]);

    const mockSupabase = supabaseManager.getClient();
    vi.mocked(mockSupabase.from as any).mockImplementation(() => ({
      select: vi.fn().mockReturnThis(),
      eq: vi.fn().mockReturnThis(),
      neq: vi.fn().mockResolvedValue({ data: [], error: null }),
      in: vi.fn().mockResolvedValue({ data: [], error: null }),
  ...
  it('sets needs_discovery=true in DB for queued documents', async () => {
    const { getFolderClaimsMap } = await import('../../src/plugins/manager.js');
    getFolderClaimsMapMock = vi.mocked(getFolderClaimsMap);
    ...
    expect(updateMock).toHaveBeenCalledWith({ needs_discovery: true });
  });
});
