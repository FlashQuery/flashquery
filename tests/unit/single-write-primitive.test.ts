...
const ALLOWED_DIRECT_WRITE_SITES = [
  ...
  },
  {
    file: 'src/mcp/tools/documents/move.ts',
    operation: 'rename',
    snippet: 'await rename(destAbsPath, sourceAbsPath);',
    reason: 'same-filesystem rollback after DB failure; EXDEV fallback must use writeVaultFile',
  },
  {
...
describe('T-U-030 single durable vault write primitive guard', () => {
  ...
  it('keeps current vault markdown write callers routed through writeVaultFile', () => {
    ...
    expect(gitSource).toMatch(
      /withAncestorDirectoryLocksShared\(lockConfig, dumpAbsPath[\s\S]*withDocumentLock\(lockConfig, dumpAbsPath[\s\S]*writeVaultFile\(dumpAbsPath, output, \{ lockConfig \}\)/
    );
