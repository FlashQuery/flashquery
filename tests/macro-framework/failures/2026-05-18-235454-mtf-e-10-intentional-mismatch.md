...
failed_at: "2026-05-18T23:54:54.863Z"
...
**INVALIDATED 2026-05-19 (Phase 6 folder review):** This record was created by a Phase 3 draft of the runner that wrote a record on every comparator divergence, regardless of `comparison:` mode. The test now uses `comparison: match_some` and is a self-test of the comparator's divergence-detection path — its declared divergences are not regressions, and per §9.6 no record should be emitted. The Phase 6 runner (cases.test.ts) correctly gates record writes behind `!cmp.ok`. Record retained for git history per §9.6 lifecycle policy.
...
```json
{
  "task_id": "9479d98a-50af-4c83-bed8-32596a4192bc",
  "result": {
    "sum": 10
  },
  "trace": [
...
- 2026-05-18T23:54:54.863Z — auto-classified by macro-framework-runner (Phase 3 draft)
