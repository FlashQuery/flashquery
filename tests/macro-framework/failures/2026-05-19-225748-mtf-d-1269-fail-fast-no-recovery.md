...
confidence: low
test_id: mtf-d-1269-fail-fast-no-recovery
test_file: cases/dispatch/1269-fail-fast-no-recovery.yml
covers: ["MTF-C-009", "MTF-D-101"]
golden_version_used: 0.3.0
golden_version_current: 0.3.0
failed_at: "2026-05-19T22:57:48.164Z"
...
Investigate the macro engine code path responsible for the error.code, side_effects.tool_call_count surface. Start from the production engine in `flashquery/src/macro/` and trace what differs from the golden's path shown in `golden_snapshot.state_notes`. If a regression is confirmed, fix the engine and re-run.

## Action log

- 2026-05-19T22:57:48.164Z — auto-classified by flashquery-macro-run (engine-bug, low confidence)
