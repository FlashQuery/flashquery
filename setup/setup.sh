...
if [ "$SUPABASE_CHOICE" = "3" ]; then
  ...
  echo "       docker compose --env-file docker/.env.docker -f docker/docker-compose.yml up -d"
  echo ""
  echo "  2. Wait ~20 seconds for all services to be healthy, then tail logs:"
  echo "       make logs"
  echo "       # FlashQuery runs inside the container — no separate npm run dev needed."
  echo ""
  echo "  3. Register FlashQuery with Claude Code:"
