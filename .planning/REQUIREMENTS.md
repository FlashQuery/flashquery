...
- [x] **REQ-001**: `fqc_embeddings` per-instance catalog table with `(instance_id, name)` uniqueness, JSONB `endpoints`, and `status` orthogonal to `source` (§6.1.1)
- [x] **REQ-002**: YAML `embeddings:` section parsed and strictly validated; no silent defaults; legacy `?? 1536` fallback removed (§6.1.2)
- [x] **REQ-003**: Config-sync inserts newly-declared YAML entries and adds their per-entry column sets + HNSW indexes (§6.1.3)
- [x] **REQ-004**: Config-sync refuses startup on in-place vector-space identity changes; benign changes apply silently with audit log (§6.1.4)
- [x] **REQ-005**: Removing an entry's YAML block deactivates it in place (status → deactivated); columns/data preserved; ERROR log per restart (§6.1.5)
...
- [x] **REQ-007**: Per-instance multi-tenancy isolation — every catalog op scoped by `instance_id` (§6.1.7)
...
- [x] **REQ-035**: `backfill_embeddings` action — embed NULL rows in scope, idempotent, dry-run + background, ends with HNSW reindex (§6.6.1)
- [x] **REQ-036**: `rebuild_embeddings` action — overwrite in-scope vectors; requires `confirm` + `max_rows`; `stale_only` / `mismatched_width_only` narrowing (§6.6.2)
...
- [x] **REQ-038**: Per-entry concurrency lock keyed on `(instance_id, embedding_name)` with heartbeat crash safety; independent across entries (§6.6.4)
- [x] **REQ-039**: Background job `abort` action — stop at checkpoint, keep embedded rows, release lock, status `aborted` (§6.6.5)
- [x] **REQ-040**: `max_rows` contract — hard ceiling (refuse before work), `0` = unlimited, backfill-optional / rebuild-required / retire-invalid (§6.6.6)
...
- [x] **REQ-042**: First-time enablement recipe (6 steps) validated as directed + integration scenarios (§6.7.1)
- [x] **REQ-043**: Legacy schema reset recipe (7 steps, wipe-and-re-establish) validated as a directed scenario (§6.7.2)
...
|-------------|-------|--------|
| REQ-001 | Phase 165 | Complete |
| REQ-002 | Phase 165 | Complete |
| REQ-003 | Phase 165 | Complete |
| REQ-004 | Phase 165 | Complete |
| REQ-005 | Phase 165 | Complete |
| REQ-006 | Phase 166 | Complete |
| REQ-007 | Phase 165 | Complete |
...
| REQ-035 | Phase 167 | Complete |
| REQ-036 | Phase 167 | Complete |
| REQ-037 | Phase 167 | Complete |
| REQ-038 | Phase 167 | Complete |
| REQ-039 | Phase 167 | Complete |
| REQ-040 | Phase 167 | Complete |
| REQ-041 | Phase 167 | Complete |
| REQ-042 | Phase 167 | Complete |
| REQ-043 | Phase 167 | Complete |
...
*Last updated: 2026-06-12 — reconciled with `Embedding Gaps.md`, the external Test Plan, current source inspection, and milestone audit. Phase 165 and Phase 167 are verification-green; the mixed core+records background job lifetime/status/abort blocker was fixed by commit `fd2407a`. REQ-043 remains complete via the designated `legacy-schema-reset.test.ts` evidence.*
