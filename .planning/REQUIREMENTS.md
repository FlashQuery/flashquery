...
- [ ] **DOC-01**: `get_document` keeps the shipped include/sections/follow_ref surface while migrating single-result errors to canonical envelopes and expected-error `isError: false` semantics.
...
- [ ] **DOC-05**: `copy_document`, `move_document`, and `list_vault` retain their existing behavior while returning structured JSON envelopes instead of prose/table text.
