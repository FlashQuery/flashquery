...
- [x] **MACRO-DISP-01**: Namespaced macro tool calls dispatch through a `(server, tool)` registry.
- [x] **MACRO-DISP-02**: Static permission pre-scan rejects denied or unknown tool references before side effects.
- [x] **MACRO-DISP-03**: Dispatch-time permission backstop rejects references that bypass pre-scan.
- [x] **MACRO-DISP-04**: `fq.call_macro` is universally unavailable from inside macros.
- [x] **MACRO-DISP-05**: Template-masqueraded tools are universally unavailable from inside macros.
- [x] **MACRO-DISP-06**: Delegated-emitted macros cannot call `fq.call_model`.
- [x] **MACRO-DISP-07**: Caller identity is derived from existing FlashQuery call context.
