...
- 🔄 **v3.3 MCP Tools Consolidation** — Phases 121-129 (Phase 129 added 2026-05-13)
...
**9 phases** | **57 requirements mapped** | Phase 129 added for post-implementation correction
...
| 129 | Correct Delegated Tier Eligibility Derivation | 1/3 | In Progress|  |
...
**Goal:** Delegated tier membership is derived from canonical tool metadata instead of a hand-maintained allow-list, closing the §3.11.1 post-implementation drift finding without changing any unrelated tool exposure behavior.

**Requirements:** POST-01 (§3.11.1 Delegated Tier Eligibility — Hand-Maintained Allow-List Drift)

**Depends on:** Phase 128

**Success Criteria** (what must be TRUE):
1. `CURRENT_DELEGATED_TIER_ORDER` and `CURRENT_DELEGATED_TIER_TOOLS` are removed from `src/mcp/tool-metadata.ts`.
2. Delegated tier eligibility is derived from `tier`, `categories`, `hostEligible`, tool status, hard exclusions, and the new optional `delegatedExclusionReason` field.
3. Data-category filtering includes `doc-read`, `doc-write`, `memory`, and `plugin`, while `llm` and `system` remain excluded from broad tier expansion.
...
Plans:
**Wave 1**
- [x] 129-01-PLAN.md — Replace delegated tier allow-list derivation with metadata-derived eligibility and unit coverage.
...
| 129 | 1 | Delegated tier derivation unit/integration coverage plus scenario/docs correction for §3.11.1 |

**Total:** 57 requirements, 57 mapped, 0 unmapped.
...
| 129 | Derived delegated tier membership, data-category filtering, `delegatedExclusionReason`, hard-exclusion/status/admin exclusions, and exact expected diff | `assembleNativeToolRegistry` tier expansion and exclusions through purpose configs | Delegated registry/listTools or equivalent MCP round-trip proving corrected tier surface | Metadata/tool-registry coverage rows, preferably new `M-` rows in directed coverage | Delegated purpose workflow proving corrected edit/list tools are accepted and dispatchable |
