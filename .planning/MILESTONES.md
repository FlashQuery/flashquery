...
## v4.0 Embedding Management & Multi-Provider Support (Shipped: 2026-06-12)

**Delivered:** Per-instance embedding catalog, multi-provider endpoint chains, per-entry vector storage, catalog-aware write/search/plugin/lifecycle paths, and operator recipes for embedding management.

**Phases completed:** 3 phases, 14 plans, 43 requirements
...
**Known deferred items at close:** 1 non-blocking tech-debt item accepted from the milestone audit: lifecycle abort marks the job aborted and releases the status-based running lock before worker checkpoint return is externally proven. Also acknowledged 2 unrelated open debug artifacts at close; see STATE.md Deferred Items.

**Key accomplishments:**

- Replaced legacy singular `embedding:` with a strict top-level `embeddings:` catalog and `fqc_embeddings` config-sync lifecycle.
- Added per-entry pgvector column sets, HNSW indexes, semantic RPCs, stamping columns, drift detection, and provider-side vector width guards.
- Built per-active-entry write fan-out with per-entry pending rows, suffixed deferred warnings, oversized-input truncation, endpoint rate limiting, and 429 backoff.
- Shipped catalog-aware semantic search with `embedding_names`, RRF fusion, deterministic tie-breaking, zero-active behavior, and partial-retriever failure handling.
- Added plugin embedding resolution and frozen per-registration choices, including plugin table column/RPC creation, write/search routing, re-registration, and legacy migration.
- Delivered `maintain_vault` lifecycle actions for backfill, rebuild, retire, status, and abort with durable jobs, locks, records-scope resolution, and operator recipe validation.
- Closed milestone audit gaps for mixed core+records background job ownership and semantic `search_records` RPC consumption.

---

## v3.9 Vault Write Coherency Locking (Shipped: 2026-06-03)

**Phases completed:** 10 phases, 41 plans, 82 tasks
**Archive:** [milestones/v3.9-ROADMAP.md](milestones/v3.9-ROADMAP.md)
...
**Phase artifacts:** [milestones/v3.9-phases/](milestones/v3.9-phases/)

**Known deferred items at close:** 1 tech-debt item accepted from the milestone audit: backup command session-capability parity remains admin-path debt; main startup and standalone scan command paths are covered.
