...
|--------|---------------------|---------|--------|-------|
| ATL-E2E-01 | `tests/e2e/call-model-agent-loop.e2e.test.ts` | `npm run test:e2e -- tests/e2e/call-model-agent-loop.e2e.test.ts -t ATL-E2E-01`; final combined E2E gate | PASS | Mode 1 envelope, `return_messages`, and raw `list_models` discovery shape. |
...
| ATL-E2E-08 | `tests/e2e/call-model-agent-loop.e2e.test.ts` | `npm run test:e2e -- tests/e2e/call-model-agent-loop.e2e.test.ts -t ATL-E2E-08`; final combined E2E gate | PASS | Capability/provider incompatibility fails before provider dispatch. |
| ATL-INT-01 | `tests/scenarios/integration/INTEGRATION_COVERAGE.md` IL-37; `llm_template_reference_freshness.yml` | `python3 tests/scenarios/integration/run_integration.py --managed llm_template_reference_freshness llm_template_document_param_freshness llm_discovery_then_call llm_mixed_reference_modes` | PASS | Reference/template freshness after document update. |
| ATL-INT-02 | `INTEGRATION_COVERAGE.md` IL-38; `llm_template_document_param_freshness.yml` | same YAML subset; final report `tests/scenarios/integration/reports/integration-report-2026-05-07-021326.md` | PASS | Document-param freshness now asserts against returned hydrated messages to avoid live-model echo variance. |
| ATL-INT-03 | `INTEGRATION_COVERAGE.md` IL-39; `llm_discovery_then_call.yml` | same YAML subset | PASS | Discovery-to-invocation closure through public `list_purposes` then `call_model`. |
...
| ATL-INT-05 | `INTEGRATION_COVERAGE.md` IL-40; `llm_mixed_reference_modes.yml` | YAML subset | PASS | Mixed path, section, pointer, alias, and `_items` template/reference modes in one flow; no active `{{id:...}}`. |
