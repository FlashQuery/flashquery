...
| 6 | Phase-specific runnable tests exist and pass for template validation, substitution, document parameters, aliases, `_items`, and public parameterized-template behavior. | VERIFIED | Local spot-checks passed: `npm test -- tests/unit/reference-resolver.test.ts tests/unit/llm-tool.test.ts` = 124/124; `npm run test:integration -- tests/integration/reference-resolver.integration.test.ts` = 8/8. Directed scenario file exists and is registered in coverage; validation records the managed scenario gate passed. |
