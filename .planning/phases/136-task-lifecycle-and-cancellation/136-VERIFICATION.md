...
| 3 | Cancellation is observed between statements, before tool calls, between loop iterations, between pipeline stages, and inside long-running builtins. | VERIFIED | `src/macro/evaluator.ts` checks `between statements`, `before statement`, `for-loop iteration`, `between pipeline stages`, and `before tool call`; `src/macro/builtins.ts` checks `inside sleep` every chunk. T-U-178 through T-U-184 pass. |
...
| `src/macro/evaluator.ts` | Cancellation signal and safe points | VERIFIED | Exports `MacroCancellationError`, maps cancellation to expected-error envelope, checks safe points, and preserves arg-evaluation-before-dispatch order. |
| `src/macro/builtins.ts` | Chunked sleep cancellation | VERIFIED | `sleep` and `slow_op` use `sleepWithCancellation()` and check `inside sleep` after 100 ms chunks. |
| `tests/unit/macro-task-registry.test.ts` | T-U-172 through T-U-177 | VERIFIED | Lifecycle tests exist and pass. |
| `tests/unit/macro-cancellation.test.ts` | T-U-178 through T-U-184 | VERIFIED | Safe-point and envelope tests exist and pass. |
...
| `tests/config/vitest.integration.config.ts` | Integration registration | VERIFIED | Includes `tests/integration/macro-concurrency.test.ts`. |
