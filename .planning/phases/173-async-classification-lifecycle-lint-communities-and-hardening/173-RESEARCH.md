...
1. **Graph lint run storage:** RESOLVED: Add a focused graph lint run-history table or schema extension sufficient for persisted payloads, latest/by-run/list-by-limit status reads, and pruning by count or age. Do not rely only on a single cursor/state row in `fqc_graph_maintenance_state`, because it stores one row per `(instance_id, scope)` and cannot naturally satisfy historical status listing/prune contracts. [VERIFIED: src/storage/supabase.ts:577] [CITED: product requirements §7.8]
2. **Community algorithm:** RESOLVED: Use a deterministic topology-only fallback for Phase 173. Do not add a new community package unless execution finds a proven local reason and runs package legitimacy/human verification. Tests assert public contracts and topology-only behavior, not Leiden internals. [CITED: product requirements §6.6.2] [CITED: product test plan §8]
3. **Worker trigger point:** RESOLVED: Implement an explicit graph worker function callable from scanner/maintenance. It must never run synchronously inside document writes, must remain queue-driven/tool-triggered rather than session state, and must surface per-run counts plus skipped-work warnings in maintenance/tool responses. [VERIFIED: src/services/scanner.ts:1263] [CITED: product requirements §3.2] [CITED: product requirements INV-06]
4. **Mock LLM fixture shape:** RESOLVED: Unit and integration tests use injected fake clients so core coverage never requires production LLM keys. YAML scenario T-Y-003 uses managed mock/fixture behavior and no production LLM credentials. [CITED: product requirements assumption 03] [CITED: product test plan §3]
