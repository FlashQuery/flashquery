...
1. **Exact job table naming and schema:** recommended as `fqc_maintenance_jobs`, but the source spec allows either advisory locks or a tracking table. [CITED: .planning/phases/167-lifecycle-operations-and-validation/167-CONTEXT.md] [ASSUMED]
2. **Batch size and estimated cost formula:** external requirements require dry-run estimated tokens/cost/wall time, but local provider cost metadata for embeddings was not found during code inspection. [CITED: external requirements §6.6.1] [VERIFIED: codebase grep] The planner should include a small design task to define conservative estimates from character count and configured endpoint metadata or return zero/unknown where the source docs permit. [ASSUMED]
3. **Legacy schema reset mechanics:** REQ-043 requires startup to fail on legacy `embedding` purpose and a seven-step wipe recipe; the planner should reread external Research §11.2 before writing the scenario because only requirements/test-plan excerpts were inspected here. [CITED: external requirements §6.7.2]
