...
| T-133-04 | Tampering | mitigate | CLOSED | `src/macro/preflight.ts:114-137` rejects invalid `input_var` arity and unsupported named args before execution; `src/macro/evaluator.ts:544-558` repeats `input_var` arity/named-arg validation on the evaluator call path; `src/macro/builtins.ts:31-39` validates `input_var` arity and key type in the builtin registry; `src/macro/builtins.ts:51-53` rejects unsupported `count` named args and wrong arity; `src/macro/builtins.ts:181-188` validates `task_id` and `list_tasks` zero-arity; `src/macro/builtins.ts:200-215` validates `sleep`/`slow_op` arity, duration type, and label type; `src/macro/builtins.ts:255-266` emits stable arity `details.reason` values through `requireArgCount`; `src/macro/builtins.ts:269-280`, `src/macro/builtins.ts:246-252`, `src/macro/builtins.ts:293-303`, and `src/macro/builtins.ts:307-314` reject unsupported named args and invalid value types with stable reasons. Tests at `tests/unit/macro-preflight.test.ts:68-86`, `tests/unit/macro-builtins.test.ts:48-58`, `tests/unit/macro-builtins.test.ts:131-151`, and `tests/unit/macro-termination.test.ts:40-57` assert representative `input_var`, `count`, and type reason stability. |
