...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 134-01-01 | 01 | 1 | MACRO-SHELL-02 | T-134-01 | Shell paths resolve under vault root; escaping paths fail with `forbidden_path`; `find` output returns vault-rooted paths | unit | `npx vitest run --config tests/config/vitest.unit.config.ts tests/unit/macro-path-wrapper.test.ts` | ✅ | ✅ green |
| 134-02-01 | 02 | 1 | MACRO-SHELL-03 | T-134-02 / T-134-03 | Forbidden mutation flags are rejected before any statement executes | unit | `npx vitest run --config tests/config/vitest.unit.config.ts tests/unit/macro-forbidden-flags.test.ts` | ✅ | ✅ green |
| 134-03-01 | 03 | 2 | MACRO-SHELL-01, MACRO-SHELL-04 | T-134-04 / T-134-05 | Only the eight read-only shell verbs execute; mutation verbs are absent; code never mutates process-global cwd | unit/static | `npx vitest run --config tests/config/vitest.unit.config.ts tests/unit/macro-shell-verbs.test.ts` | ✅ | ✅ green |
| 134-04-01 | 04 | 3 | MACRO-SHELL-05 | T-134-06 | `_exists()` resolves through native/broker layers and unknown underscore methods fail at runtime | unit | `npx vitest run --config tests/config/vitest.unit.config.ts tests/unit/macro-introspection.test.ts` | ✅ | ✅ green |
| 134-05-01 | 05 | 4 | MACRO-SHELL-01 through MACRO-SHELL-05 | T-134-01 through T-134-06 | Complete shell/vault/pre-scan/introspection contract passes as a phase gate | unit/static | `npx vitest run --config tests/config/vitest.unit.config.ts tests/unit/macro-path-wrapper.test.ts tests/unit/macro-shell-verbs.test.ts tests/unit/macro-forbidden-flags.test.ts tests/unit/macro-introspection.test.ts` | ✅ | ✅ green |
...
- [x] `tests/unit/macro-path-wrapper.test.ts` — stubs and assertions for T-U-137 through T-U-142.
- [x] `tests/unit/macro-shell-verbs.test.ts` — stubs and assertions for T-U-126 through T-U-136, T-U-143, and T-U-151.
- [x] `tests/unit/macro-forbidden-flags.test.ts` — stubs and assertions for T-U-144 through T-U-150.
- [x] `tests/unit/macro-introspection.test.ts` — stubs and assertions for T-U-152 through T-U-155.
- [x] `src/macro/path-wrapper.ts` — vault-root path resolver and host-to-macro path helper.
- [x] `src/macro/shell-verbs.ts` — exact eight read-only shell builtin registrations.
- [x] `src/macro/forbidden-flag-scan.ts` — AST pre-scan for forbidden `sed` and `find` mutation flags.
- [x] `src/macro/introspection.ts` — `_exists()` native/broker resolver.
- [x] Dependencies available: `shelljs`, `fast-glob`, and `@types/shelljs` or a documented local typed adapter.
...
Rerun evidence:
- `npm test -- --run tests/unit/macro-path-wrapper.test.ts tests/unit/macro-shell-verbs.test.ts tests/unit/macro-forbidden-flags.test.ts tests/unit/macro-introspection.test.ts ...`: included in macro validation slice, 21 files passed / 244 tests passed.
