...
| Property | Value |
|----------|-------|
| **Framework** | Vitest 4.1.1 plus FlashQuery macro-framework, directed, and YAML scenario harnesses |
| **Config file** | `tests/config/vitest.unit.config.ts`, `tests/config/vitest.macro-framework.config.ts`, `tests/config/vitest.integration.config.ts`, `tests/config/vitest.e2e.config.ts` |
...
| **Full suite command** | `npm run build && npm test && npm run test:macro-framework && npm run test:integration && npm run test:e2e` plus Phase E directed/YAML scenario commands |
...
- **Before `$gsd-verify-work`:** Full suite, macro-framework, Phase E directed scenarios, Phase E YAML scenarios, and required coverage ledger updates must be green.
...
| 143-02-01 | 02 | 1 | REQ-103 | SELF-01 | `_self` is source_ref-only, read-only, and snapshot-based | unit + macro-framework | `npm test -- --run tests/unit/macro-self.test.ts tests/unit/macro-source-ref.test.ts && npm run test:macro-framework` | ❌ W0 | ⬜ pending |
...
| 143-03-01 | 03 | 2 | REQ-104 | PARSE-01 | `continue` and `break` are parse-time invalid outside loops and runtime-valid inside loops | unit + macro-framework | `npm test -- --run tests/unit/macro-parser.test.ts tests/unit/macro-evaluator.test.ts && npm run test:macro-framework` | ✅ | ⬜ pending |
...
- [ ] Macro testing framework cases updated from Phase 142 workaround/pending notes to real Phase 143 behavior where applicable: `_self` in `tests/macro-framework/cases/isolation/`, `continue`/`break` in `tests/macro-framework/cases/control-flow/`.
