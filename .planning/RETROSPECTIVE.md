...
## Milestone: v4.0 — Embedding Management & Multi-Provider Support
...
- Per-instance embedding catalog with multiple active embeddings, purpose tags, schema validation, and migrated configuration.
- Per-entry pgvector storage, stamping, HNSW indexes, generated match RPCs, drift detection, and provider/dimension guards.
- Per-active-entry write fan-out with pending/deferred handling, truncation metadata, rate limiting, 429 protection, and admin disable paths.
- Catalog-aware search across memory, documents, records, and plugins with explicit dimensions, reciprocal-rank fusion, partial-failure reporting, and zero-active handling.
- Plugin embedding choice resolution with frozen defaults, plugin table RPC generation, plugin write/search paths, and migration support.
- `maintain_vault` lifecycle actions for backfill, rebuild, retire, abort, records coverage, mixed core+records jobs, locks, status, and operator recipes.

### What Worked

- The source requirements and test plan stayed explicit enough to trace 43 requirements through implementation and audit.
- Red/green regressions for the audit findings kept fixes focused instead of creating another broad phase.
- Cross-phase audit caught a real mixed lifecycle background-job ownership bug that phase-local checks had missed.
- Follow-up forensic review reconciled stale phase verification text against the gap log instead of treating every stale discrepancy as new scope.

### What Was Inefficient

- Phase-local gap and verification documents drifted after later remediation, causing repeated closeout confusion.
- Shared `.env.test` database state and vector-column pressure made some operator recipe evidence harder to prove cleanly.
- `gsd-sdk` was unavailable during closeout, so archive and state updates had to be applied manually.
- Quiet E2E progress made long-running tests look stalled until heartbeat output was added.

### Patterns Established

- Mixed lifecycle operations need wrapper-owned public job finalization when core and records helpers share one job.
- Record semantic search should consume generated `match_records_*` RPCs when lifecycle owns those RPCs and indexes.
- Phase verification documents must be synced after external remediation, especially before milestone audit.
- Audit deltas should be compared against the product gap log before opening more implementation scope.

### Key Lessons

1. Background-job regressions need completion ordering assertions, not only job-id reuse.
2. Stale verification prose can create false closeout blockers even when implementation and tests are green.
3. Spec-backed failing tests are the right response to audit discrepancies; passing tests should not be tuned unless the test is demonstrably wrong.
4. Schema-heavy integration suites need stronger database hygiene to avoid noisy exhaustion and isolation failures.

### Cost Observations

- Model mix: codex/sonnet-class implementation with focused debugger, integration-checker, and audit agents.
- Notable: focused TDD fixes for mixed job finalization and record RPC consumption resolved audit debt without opening a new broad milestone phase.
